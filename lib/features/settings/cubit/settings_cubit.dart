...
class SettingsCubit extends Cubit<SettingsState> {
  ...
  /// Toggle Monday fasting notification
  Future<void> toggleMondayFasting(bool enabled) async {
    debugPrint('🍽️ CUBIT: toggleMondayFasting called with enabled: $enabled');

    if (state is SettingsLoaded) {
      final currentSettings = (state as SettingsLoaded).settings;
      final newNotificationSettings = currentSettings.notificationSettings
          .copyWith(mondayFastingEnabled: enabled);

      await setNotificationSettings(newNotificationSettings);

      // Schedule or cancel the notification
      await _notificationService.scheduleAllFastingNotifications(
        mondayEnabled: enabled,
        thursdayEnabled: newNotificationSettings.thursdayFastingEnabled,
        notificationTime: newNotificationSettings.fastingNotificationTime,
        vibration: newNotificationSettings.fastingVibration,
      );

      debugPrint('✅ CUBIT: Monday fasting notification toggled successfully');
    }
  }
  ...
  Future<void> toggleThursdayFasting(bool enabled) async {
    ...
    if (state is SettingsLoaded) {
      final currentSettings = (state as SettingsLoaded).settings;
      final newNotificationSettings = currentSettings.notificationSettings
          .copyWith(thursdayFastingEnabled: enabled);

      await setNotificationSettings(newNotificationSettings);

      // Schedule or cancel the notification
      await _notificationService.scheduleAllFastingNotifications(
        mondayEnabled: newNotificationSettings.mondayFastingEnabled,
        thursdayEnabled: enabled,
        notificationTime: newNotificationSettings.fastingNotificationTime,
        vibration: newNotificationSettings.fastingVibration,
      );

      debugPrint('✅ CUBIT: Thursday fasting notification toggled successfully');
    }
  }

  /// Update fasting notification time
  Future<void> setFastingNotificationTime(String time) async {
    debugPrint('🍽️ CUBIT: setFastingNotificationTime called with time: $time');

    if (state is SettingsLoaded) {
      final currentSettings = (state as SettingsLoaded).settings;
      final newNotificationSettings = currentSettings.notificationSettings
          .copyWith(fastingNotificationTime: time);

      await setNotificationSettings(newNotificationSettings);

      // Reschedule all fasting notifications with new time
      await _notificationService.scheduleAllFastingNotifications(
        mondayEnabled: newNotificationSettings.mondayFastingEnabled,
        thursdayEnabled: newNotificationSettings.thursdayFastingEnabled,
      ...
      debugPrint('✅ CUBIT: Fasting notification time updated successfully');
    }
  }
  ...
  Future<void> toggleFastingVibration(bool enabled) async {
    ...
    if (state is SettingsLoaded) {
      final currentSettings = (state as SettingsLoaded).settings;
      final newNotificationSettings = currentSettings.notificationSettings
          .copyWith(fastingVibration: enabled);

      await setNotificationSettings(newNotificationSettings);

      // Reschedule notifications with new vibration setting
      await _notificationService.scheduleAllFastingNotifications(
        mondayEnabled: newNotificationSettings.mondayFastingEnabled,
        thursdayEnabled: newNotificationSettings.thursdayFastingEnabled,
        notificationTime: newNotificationSettings.fastingNotificationTime,
      ...
      debugPrint('✅ CUBIT: Fasting vibration toggled successfully');
    }
  }
