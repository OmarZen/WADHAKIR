#!/usr/bin/env pwsh
# Shorebird iOS Patch Script
# This script creates and deploys an over-the-air (OTA) patch for iOS
...
Write-Host "🔁 Starting Shorebird iOS Patch Deployment..." -ForegroundColor Cyan
...
# Execute the patch
try {
    Write-Host "📦 Creating iOS Patch..." -ForegroundColor Green
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "✅ iOS Patch Deployed Successfully!" -ForegroundColor Green
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "⚠️  iOS Performance Note:" -ForegroundColor Yellow
        Write-Host "   Changed code runs in a Dart interpreter (slightly slower)." -ForegroundColor White
        Write-Host "   This is usually unnoticeable unless changing performance-critical code." -ForegroundColor White
        Write-Host ""
        ...
        Write-Host "   • iOS builds require macOS" -ForegroundColor White
        exit 1
    }
} catch {
    Write-Host ""
